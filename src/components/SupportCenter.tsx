      ...
      <div className="mx-auto min-h-[100svh] w-full max-w-[430px] border-x border-white/5 bg-[#09090a] pb-8">
        ...
        <div className="p-4">
          ...
          {mode === "submit" && <section className="mt-5 space-y-4">
            ...
            <label className="flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border border-dashed border-white/15 px-4 text-sm text-white/65"><Paperclip className="h-4 w-4 text-gold" /><span className="min-w-0 flex-1 truncate">{file?.name ?? "Attach screenshot, PDF, or text file"}</span>{file && <button type="button" onClick={(event) => { event.preventDefault(); setFile(null); }}><X className="h-4 w-4" /></button>}<input type="file" accept="image/png,image/jpeg,image/webp,application/pdf,text/plain" className="hidden" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /></label>

...
export function LockerBeatCard({ beat, previewing, previewProgress, onPreview, onUse, onRemove }: { beat: BeatLockerRow; previewing: boolean; previewProgress: number; onPreview: () => void; onUse: () => void; onRemove: () => void }) {
  const favorite = beat.license?.toLowerCase() === "favorite";
  return (
    <article className="rounded-2xl border border-white/10 bg-[#111113] p-3">
      <div className="flex gap-3">
        <button type="button" onClick={onPreview} className="relative grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-xl border border-gold/20" style={{ background: lockerBeatArt(beat) }} aria-label={previewing ? `Pause ${beat.title}` : `Preview ${beat.title}`}><span className="grid h-9 w-9 place-items-center rounded-full border border-gold/30 bg-black/65 text-gold backdrop-blur-sm">{previewing ? <Pause className="h-4 w-4 fill-current" /> : <Headphones className="h-4 w-4" />}</span><span className="absolute inset-x-2 bottom-2 h-0.5 overflow-hidden rounded-full bg-white/20"><span className="block h-full bg-gold transition-[width] duration-150" style={{ width: `${previewProgress}%` }} /></span></button>

    ...
    <main className="min-h-svh bg-[#060607] text-white">
      <div className="mx-auto min-h-svh w-full max-w-[430px] border-x border-white/8 bg-[#09090a] pb-24">
        ...
        {loading ? <div className="px-5 py-12 text-center text-sm text-muted-foreground">Opening your private rooms...</div> : visibleRequests.length === 0 ? (
          <div className="mx-5 rounded-2xl border border-white/10 bg-[#111113] p-6 text-center"><Handshake className="mx-auto h-7 w-7 text-gold" /><h2 className="mt-3 text-lg font-semibold">{producerContext ? "No artist requests yet" : "No requests yet"}</h2><p className="mt-2 text-sm text-muted-foreground">{producerContext ? "New briefs will appear here when artists request one of your published services." : "Your producer requests and accepted sessions will appear here."}</p><Link href={producerContext ? "/producer?panel=service" : "/?view=market"} className="mt-5 inline-flex min-h-11 items-center rounded-xl border border-gold/25 px-4 text-sm font-semibold text-gold">{producerContext ? "Manage services" : "Explore producers"}</Link></div>
        ) : (
          <>

...
const schema = z.object({
  plan_id: z.string().regex(/^artist_[a-z0-9_]+$/),
...
export async function POST(request: Request) {
  ...
  if (audience !== "artist") return NextResponse.json({ error: "Producer HQ does not require a paid membership." }, { status: 400 });
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${appUrl}/studio?view=profile&subscription=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${appUrl}/studio?view=profile&subscription=cancelled`,

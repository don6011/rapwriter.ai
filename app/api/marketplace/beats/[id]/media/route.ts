...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  const { data: beat, error } = await supabase
    .from("producer_beats")
    .select("owner_id, audio_path, artwork_path, metadata")
    .eq("id", beatId)
    .eq("status", "approved")
    .maybeSingle();
  ...
  const path = kind === "audio" && isOwnedProducerPreviewPath(previewPath, beat.owner_id, beat.audio_path)
    ? previewPath
    : kind === "artwork"
      ? beat.artwork_path
      : null;
  ...
  const { data, error: signError } = await supabase.storage.from(PRODUCER_BEAT_BUCKET).createSignedUrl(path, 60 * 5);
